...
@guesses=("crazyguess.inc","guesslambert.inc","guesslambert2.inc","guesstangential.inc","guesstangential2.inc","guesstangential2m.inc","guesstangential3.inc","guesstangential3m.inc","NOGUESS");
$mass=1;
$min=3652.5;
$total=0;
$infis=0;
		...
		foreach (<MAINOLD>){
		    if (/param depart/) { 
		     ...
		     $_="param M	     :=   (".($mass*2000).");	\n";
		      ...
		      $_="var tI	     :=   ".($data[$j*2]-$miss*100).";		#Departure time (MJD2000)\n";
		    } 
		    elsif (/var tT/) {
		      $_="var tT	     :=   ".($data[$j*2+1]+$corrections[$l]).";		#Arrival time (MJD2000)\n";
		    }
		    if ((/include/) & (/guess/)) {
...
$objfun=0.81/($mass/0.93);
print OUTPUT "VALUE TO BEAT IN THE FLYBY ".$objfun."\n";
print "=============================================================================\n";
...
sub infeasible {
    ...
    @LOG=<LOGFILE>;
    close(LOGFILE);
    if (($LOG[$#LOG-26] =~ /i/)  || !($LOG[$#LOG-26] =~ /\([+-\d.Ee]+\)/) ){
    return 1;
    }
    else {
    return 0;
